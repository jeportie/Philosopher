...
#    Updated: 2024/09/19 13:26:54 by jeportie         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
uml:
	@if sudo -n true 2>/dev/null; then \
		./make_interface/exec/interactive_select_sudo; \
	else \
		./make_interface/exec/interactive_select; \
	fi

build:
	@./make_interface/exec/config/build
	make update

update:
	@./make_interfacei/exec/update_makefile.sh
...
download-script:
	@if [ ! -d make_interface ]; then \
		...
		chmod +x make_interface/exec/make_prompt; \
